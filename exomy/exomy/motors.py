...
class Motors():
    ...
    def __init__(self, parameters):
        ...
        self.driving_pwm_range = [None] * 6

        self.driving_pwm_range[self.FL] = parameters['drive_pwm_range_fl']
        self.driving_pwm_range[self.FR] = parameters['drive_pwm_range_fr']
        self.driving_pwm_range[self.CL] = parameters['drive_pwm_range_cl']
        self.driving_pwm_range[self.CR] = parameters['drive_pwm_range_cr']
        self.driving_pwm_range[self.RL] = parameters['drive_pwm_range_rl']
        self.driving_pwm_range[self.RR] = parameters['drive_pwm_range_rr']

        # Set the GPIO to software PWM at 'Frequency' Hertz
        self.driving_motors = [None] * 6
        self.duty_cycle = [None] * 6
...
#
            ...
            self.duty_cycle[wheel_name] = int(self.driving_pwm_neutral +
                             driving_command[wheel_name]/100.0 * self.driving_pwm_range[wheel_name] * self.wheel_directions[wheel_name])

            if driving_command[0] < 0 and driving_command[1] > 0 :#and wheel_name in [self.FL, self.CL, self.RL]:
                self.duty_cycle[wheel_name] = int(self.driving_pwm_neutral +(driving_command[wheel_name]*1.7)/100.0 * self.driving_pwm_range[wheel_name] * self.wheel_directions[wheel_name])
                if wheel_name in [self.FL, self.CL, self.RL]:
                    self.duty_cycle[wheel_name] = int(self.driving_pwm_neutral +(driving_command[wheel_name]*2.)/100.0 * self.driving_pwm_range[wheel_name] * self.wheel_directions[wheel_name])
            if driving_command[0] > 0 and driving_command[1] < 0 :
                self.duty_cycle[wheel_name] = int(self.driving_pwm_neutral + (driving_command[wheel_name]*1.10)/100.0 * self.driving_pwm_range[wheel_name] * self.wheel_directions[wheel_name])

            self.pwm.set_pwm(motor_pin, 0, self.duty_cycle[wheel_name])
    def stopMotors(self):
